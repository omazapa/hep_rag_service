...
def test_method_extraction():
    ...
    try:
        ...
        print(
            f"Documents with MEMBERS section: {with_members['count']} ({with_members['count']/total['count']*100:.1f}%)"
        )
        print(
            f"Documents with SIGNATURE section: {with_signature['count']} ({with_signature['count']/total['count']*100:.1f}%)"
        )
